...
<template>
  ...
  <div v-if="!isValid">
    ...
    <Button as-child class="mt-3 flex"
      ><Link :href="tuyau.$url('auth.forgot-password.render')"
        >Request New Password Reset Link</Link
      ></Button
    >
  </div>
  ...
  <form
    v-else
    @submit.prevent="
      form.post(tuyau.$url('auth.forgot-password.update'), {
  ...
  >
    <div class="grid gap-3">
      <FormInput label="Email" type="email" :model-value="email!" :required="true" disabled />
